...
${localSafeReply(text)}`,createdAt:now(),model:"ACC Local Fallback"});
  ...
  const render=()=>{
    ...
    const productionRender=ui.tab==="production";
    ...
    bindEvents();
    window.dispatchEvent(new CustomEvent("acc-core-rendered",{detail:{tab:ui.tab,productionTab:ui.productionTab,channelId:state.activeChannelId}}));
    requestAnimationFrame(()=>{if(!productionRender)scrollTo(0,scroll);const list=document.getElementById("ai-message-list");if(list)list.scrollTop=list.scrollHeight;const terminal=document.getElementById("mission-terminal-log");if(terminal&&(ui.gm5Running||ui.gm5Error||ui.gm5Stage==="DONE"))terminal.scrollTop=terminal.scrollHeight;});
  };
