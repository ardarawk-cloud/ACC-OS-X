// ACC OS X — BUILD 257.6 PRODUCTION GATE + BUILD 256 SYSTEM ORCHESTRATOR ROUTE
// Adds KAI Produce Copilot as an independent semi-automatic K/P/C/N lane above Automatic Mission.
// Automatic research/material/poster/caption/QC/publishing routing remains intact.
...
import copilotWorker from "./worker-produce-copilot-v2576.js";
...
const PACKAGE_REVISION="BUILD257_6_PRODUCE_COPILOT";
const GATE_REVISION="BUILD257_6_PRODUCTION_GATE";
...
function stageOf(body){
  ...
  if(s==="PRODUCE_COPILOT")return"COPILOT";
  if(["SYSTEM","COPILOT","RESEARCH","SCRIPT","POSTER","CAPTION","QC","PUBLISHING"].includes(s))return s;
  const joined=(Array.isArray(body?.messages)?body.messages:[]).map(m=>text(m?.content)).join("\n");
  const m=joined.match(/(?:^|\n)STAGE:\s*(SYSTEM|COPILOT|RESEARCH|SCRIPT|POSTER|CAPTION|QC|PUBLISHING)\b/i);
  if(m)return String(m[1]).toUpperCase();
  if(/KAI PRODUCE COPILOT/i.test(joined))return"COPILOT";
