// ACC OS X — BUILD 257.7 PRODUCE COPILOT STABLE DOM
// Rebuilt rendering model: controls and chat container are mounted once per channel.
// Messages append/update in place. K/P/C/N never rebuilds the whole panel.
(() => {
  "use strict";
  ...
  const STYLE_ID = "acc-produce-copilot-style-v2577";
  ...
  function ensureStyle(){
    ...
    document.getElementById("acc-produce-copilot-style")?.remove();
  ...
  function messageHtml(m){
    if(m.type==="poster"&&m.image)return `<div class="acc-copilot-msg kai" data-copilot-message-id="${esc(m.id)}"><div class="acc-copilot-bubble"><span class="acc-copilot-label">KAI // POSTER</span>${esc(m.content||"Poster siap.")}<img class="acc-copilot-poster" data-copilot-poster="${esc(m.id)}" src="${m.image}" alt="Poster ${esc(m.channelName||"")}"></div></div>`;
    const label=m.role==="user"?"OWNER":"KAI";return `<div class="acc-copilot-msg ${m.role==="user"?"user":"kai"}" data-copilot-message-id="${esc(m.id)}"><div class="acc-copilot-bubble"><span class="acc-copilot-label">${label}</span>${esc(m.content)}</div></div>`;
  }
  ...
  function shellHtml(profile,row){
    const published=txt(row.package?.publishedPostId);return `<div class="card acc-copilot-card mono">
      ...
      <div class="acc-copilot-status" id="acc-copilot-status"></div><button class="acc-copilot-publish" id="acc-copilot-publish" ${canPublish(row)?"":"disabled"}>${published?`PUBLISHED ✅ ${esc(published)}`:"⚡ PUBLISH APPROVED PACKAGE"}</button>
      <div class="acc-copilot-note">Runtime ini tidak me-render ulang panel saat K/P/C/N. Hanya pesan/status yang berubah.</div></div>`;
  }
  ...
  function syncMessages(row){
    ...
    if(!row.messages.length){if(!chat.querySelector(".acc-copilot-empty"))chat.innerHTML='<div class="acc-copilot-empty">Chat KAI membaca channel yang sedang dipilih.<br>K = konten • P = poster • C = caption • N = next.<br>Kamu juga bisa ngobrol / revisi dengan kalimat biasa.</div>';return;}
    ...
    for(const m of row.messages){
      ...
      if(m.type==="poster"&&m.image){const img=node?.querySelector("img[data-copilot-poster]");if(img&&img.src!==m.image)img.src=m.image;bindPoster(img);}
    }
  }
  function syncMeta(profile,row){
    const name=document.getElementById("acc-copilot-channel-name"),meta=document.getElementById("acc-copilot-channel-meta"),count=document.getElementById("acc-copilot-context-count"),publish=document.getElementById("acc-copilot-publish");
    ...
    if(publish){const published=txt(row.package?.publishedPostId);publish.textContent=published?`PUBLISHED ✅ ${published}`:"⚡ PUBLISH APPROVED PACKAGE";publish.disabled=!canPublish(row);}
  }
  function bindPanel(){
  ...
  function mount(){
    ...
    const profile=currentProfile();if(!profile.id)return;const row=lane(profile.id);
    let section=document.getElementById(PANEL_ID);
    if(!section){section=document.createElement("section");section.id=PANEL_ID;section.className="section mono acc-copilot-section";section.dataset.channelId=profile.id;section.innerHTML=shellHtml(profile,row);missionSection.parentElement.insertBefore(section,missionSection);bindPanel();syncMessages(row);return;}
    if(section.dataset.channelId!==profile.id){section.dataset.channelId=profile.id;section.dataset.bound="";section.innerHTML=shellHtml(profile,row);if(section.nextElementSibling!==missionSection)missionSection.parentElement.insertBefore(section,missionSection);bindPanel();syncMessages(row);return;}
  ...
  function requestContext(profile,row,command){return {workerTask:{id:id("copilot"),stage:"COPILOT",workerType:"KAI_PRODUCE_COPILOT",workerName:"KAI PRODUCE COPILOT",goal:"Semi-automatic owner production",source:"OWNER_CHAT",autoApply:false},profile:{id:profile.id,code:profile.code,name:profile.name,platform:profile.platform},contexts:profile.contexts,copilot:{command,research:profile.research,material:row.package?.material||"",caption:row.package?.caption||"",history:row.messages.slice(-8).filter(m=>m.type!=="poster").map(m=>({role:m.role==="user"?"user":"assistant",content:m.content}))},client:{revision:REVISION,language:"id-ID",timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||"Asia/Makassar"}};}
  async function callCopilot(profile,row,command){const body={messages:[{role:"user",content:command}],context:requestContext(profile,row,command)},access=localStorage.getItem(AI_ACCESS_KEY)||"",headers={"Content-Type":"application/json","Accept":"application/json"};if(access)headers["X-ACC-Access-Code"]=access;const response=await fetch("/api/acc-ai",{method:"POST",headers,body:JSON.stringify(body)}),data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data?.error||data?.errorDetail?.message||`HTTP ${response.status}`);return data;}
  ...
  async function composePoster(profile,data){
    const hero=`data:image/jpeg;base64,${data.imageBase64}`,img=await loadImage(hero),canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1920;const ctx=canvas.getContext("2d",{alpha:false});ctx.fillStyle="#050914";ctx.fillRect(0,0,1080,1920);const targetH=1220,scale=Math.max(1080/img.width,targetH/img.height),w=img.width*scale,h=img.height*scale;ctx.drawImage(img,(1080-w)/2,(targetH-h)/2,w,h);
    let g=ctx.createLinearGradient(0,600,0,1500);g.addColorStop(0,"rgba(3,7,18,.02)");g.addColorStop(.48,"rgba(3,7,18,.60)");g.addColorStop(1,"rgba(3,7,18,.99)");ctx.fillStyle=g;ctx.fillRect(0,0,1080,1530);ctx.fillStyle="rgba(4,8,18,.96)";ctx.fillRect(0,1180,1080,740);ctx.fillStyle="#62efba";ctx.fillRect(54,70,8,132);ctx.fillStyle="#ffffff";ctx.font="900 46px Arial,sans-serif";ctx.fillText(profile.name.toUpperCase(),84,84);ctx.fillStyle="#9cb0c8";ctx.font="800 22px Arial,sans-serif";ctx.fillText(`${profile.code||profile.id} • ${profile.platform}`.toUpperCase(),86,146);
    const badge=txt(data.plan?.badge)||"ACC STUDIO";ctx.font="900 22px Arial,sans-serif";const bw=Math.min(460,ctx.measureText(badge).width+40);ctx.fillStyle="#62efba";ctx.fillRect(58,1080,bw,50);ctx.fillStyle="#04100d";ctx.fillText(badge.toUpperCase(),78,1093);const headline=txt(data.plan?.headline)||profile.name;ctx.fillStyle="#ffffff";ctx.font="900 68px Arial,sans-serif";wrapLines(ctx,headline,950,4).forEach((line,i)=>ctx.fillText(line,58,1245+i*80));const sub=txt(data.plan?.subhead);if(sub){ctx.fillStyle="#b8c4d3";ctx.font="600 30px Arial,sans-serif";wrapLines(ctx,sub,950,3).forEach((line,i)=>ctx.fillText(line,58,1590+i*42));}ctx.fillStyle="#62efba";ctx.fillRect(58,1810,250,4);ctx.fillStyle="#8ea0b8";ctx.font="800 19px Arial,sans-serif";ctx.fillText("ACC OS X • KAI PRODUCE COPILOT",58,1840);ctx.fillStyle="#ffffff";ctx.font="800 18px Arial,sans-serif";ctx.textAlign="right";ctx.fillText("AM STUDIO",1022,1840);ctx.textAlign="left";return canvas.toDataURL("image/jpeg",.88);
  }
  async function execute(command){
    const profile=currentProfile();if(!profile.id)return;const row=lane(profile.id),clean=txt(command);if(!clean)return;addMessage(profile.id,"user",clean);refresh();const op=/^(k|konten|content)\b/i.test(clean)?"K":/^(p|poster)\b/i.test(clean)?"P":/^(c|caption)\b/i.test(clean)?"C":/^(n|next|lanjut)\b/i.test(clean)?"N":"CHAT";
    if(op==="P"&&!txt(row.package.material)){addMessage(profile.id,"kai","K belum ada. Buat/approve konten dulu dengan K.");refresh();return;}if(op==="C"&&!txt(row.package.material)){addMessage(profile.id,"kai","K belum ada. Buat/approve konten dulu dengan K.");refresh();return;}setBusy(true,op==="P"?"KAI sedang membuat poster asli…":"KAI sedang menyusun output…");
    try{const data=await callCopilot(profile,row,clean);if(data.kind==="poster"){const dataUrl=await composePoster(profile,data);row.package.posterDataUrl=dataUrl;row.package.posterBase64=dataUrl.split(",")[1]||"";row.package.posterHeadline=txt(data.plan?.headline);row.package.posterSubhead=txt(data.plan?.subhead);row.package.publishedPostId="";row.package.publishedAt="";addMessage(profile.id,"kai",data.reply||"Poster siap.","poster",{image:dataUrl,channelName:profile.name});}else if(data.kind==="caption"){row.package.caption=txt(data.reply);row.package.publishedPostId="";row.package.publishedAt="";addMessage(profile.id,"kai",data.reply);}else if(data.kind==="material"){row.package.material=txt(data.reply);row.package.caption="";row.package.posterBase64="";row.package.posterDataUrl="";row.package.posterHeadline="";row.package.posterSubhead="";row.package.publishedPostId="";row.package.publishedAt="";addMessage(profile.id,"kai",data.reply);}else addMessage(profile.id,"kai",data.reply||"Siap.");row.updatedAt=now();saveStore();}catch(error){addMessage(profile.id,"kai",`ERROR // ${String(error?.message||error)}`);}finally{refresh();setBusy(false);setStatus("");}
  }
  ...
  async function publishApproved(){
    const profile=currentProfile(),row=lane(profile.id),pkg=row.package||{};if(!txt(pkg.posterBase64)||!txt(pkg.caption))return setStatus("Poster + caption harus siap dulu.",true);if(txt(pkg.publishedPostId))return;const main=readMain(),target=publishTarget(main,profile.id);if(!target)return setStatus("Meta Page channel ini belum di-map di Publishing Hub.",true);const access=localStorage.getItem(PUBLISH_ACCESS_KEY)||localStorage.getItem(AI_ACCESS_KEY)||"";if(!access)return setStatus("Connector access belum tersimpan di perangkat.",true);const endpoint=localStorage.getItem(PUBLISH_ENDPOINT_KEY)||DEFAULT_PUBLISH_ENDPOINT,jobId=id("copilot_publish"),stamp=Date.now(),platform=String(profile.platform||"Facebook").toUpperCase(),job={id:jobId,sourceWorkflowId:`COPILOT:${profile.id}`,sourceWorkflowRunKey:`copilot:${profile.id}:${stamp}`,sourceTaskId:null,channelId:profile.id,channelName:profile.name,workspaceId:profile.workspaceId||main.activeWorkspaceId||"acc-enterprise",platform,status:"QUEUED",attempts:1,idempotencyKey:`copilot:${profile.id}:${stamp}:${platform.toLowerCase()}`,externalPostId:null,publishedAt:null,error:null,createdAt:now(),updatedAt:now(),connector:target.connector||"META_FACEBOOK",sourceAssetId:null},payload={...job,target,pageId:target.pageId||null,pageName:target.pageName||null,instagramAccountId:target.instagramAccountId||null,content:{message:pkg.caption,mediaUrl:null,imageBase64:pkg.posterBase64,mimeType:"image/jpeg"},clientRevision:REVISION,mediaSource:"PRODUCE_COPILOT_BASE64"};setBusy(true,"Mengirim approved package ke Meta connector…");
    try{const response=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json","X-ACC-Access-Code":access},body:JSON.stringify(payload)}),data=await response.json().catch(()=>({}));if(!response.ok||!data.ok){const e=data?.error||{};throw new Error(e.message||e.code||data.message||`HTTP ${response.status}`);}const postId=txt(data.externalPostId||data.postId||data.id||data.result?.id)||"PUBLISHED";pkg.publishedPostId=postId;pkg.publishedAt=now();row.updatedAt=now();addMessage(profile.id,"kai",`PUBLISH SUCCESS ✅\n${profile.name}\nPOST ID: ${postId}`);saveStore();}catch(error){addMessage(profile.id,"kai",`PUBLISH FAILED // ${String(error?.message||error)}`);}finally{refresh();setBusy(false);setStatus("");}
  }
