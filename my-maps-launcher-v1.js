// KAI ONE — ACC OS X MY MAPS launcher v16
// Permanent source fix: each Roblox map uses its own materialized JPEG file.
// No runtime sprite, canvas crop, data URI, Blob URL, or Android native asset dependency.
// Legacy deployment marker retained intentionally: KAI_ONE_MY_MAPS_V10_NATIVE_ANDROID_ASSET
(() => {
  "use strict";

  const REVISION = "KAI_ONE_MY_MAPS_V16_INDIVIDUAL_FILES";
  const ROOT_ID = "acc-my-maps";
  const STYLE_ID = "acc-my-maps-v16-style";
  const ICON_REV = "KAI_ONE_MY_MAPS_V16_INDIVIDUAL_FILES";

  const MAPS = [
    {key:"bbya-social-hub",title:"BBYA Social Hub",placeId:"131894120482837",accent:"#d946ef",fallback:"BBYA",icon:"bbya-social-hub.jpg"},
    {key:"zona-perang",title:"Zona Perang",placeId:"85866320744490",accent:"#ef4444",fallback:"ZP",icon:"zona-perang.jpg"},
    {key:"after-school-city",title:"After School City",placeId:"121603385909425",accent:"#facc15",fallback:"ASC",icon:"after-school-city.jpg"},
    {key:"becak-e-bike",title:"Becak E-bike",placeId:"80994730522893",accent:"#f59e0b",fallback:"BE",icon:"becak-e-bike.jpg"},
    {key:"wonderpocket",title:"WONDERPOCKET",placeId:"124843214013484",accent:"#a78bfa",fallback:"WP",icon:"wonderpocket.jpg"},
    {key:"track-01",title:"Track 01",placeId:"79748872921213",accent:"#dc2626",fallback:"T01",icon:"track-01.jpg"},
    {key:"gunung-bbya",title:"Gunung BBYA",placeId:"82661754996018",accent:"#22c55e",fallback:"GB",icon:"gunung-bbya.jpg"},
    {key:"lost-found-night-shift",title:"Lost & Found: Night Shift",placeId:"93699016600671",accent:"#f97316",fallback:"LF",icon:"lost-found-night-shift.jpg"}
  ];

  function iconUrl(map){
    return `./assets/app-icons/my-maps/${map.icon}?rev=${ICON_REV}`;
  }
  ...
  function ensureStyle(){
    ...
    style.textContent = `
      ...
      #${ROOT_ID} .acc-map-fallback-v16{position:absolute;inset:0;display:grid;place-items:center;color:var(--map-accent);font-size:.92rem;font-weight:950;letter-spacing:-.04em;z-index:0}
      #${ROOT_ID} .acc-map-logo-v16{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;max-width:none!important;object-fit:cover!important;display:block!important;z-index:2!important;opacity:0!important;pointer-events:none!important}
      #${ROOT_ID} .acc-map-icon[data-image-ready="1"] .acc-map-logo-v16{opacity:1!important}
      #${ROOT_ID} .acc-map-icon[data-image-ready="1"] .acc-map-fallback-v16{opacity:0!important}
  ...
  function tile(map){
    ...
    button.innerHTML = `
      <div class="acc-map-icon" data-image-ready="0">
        <span class="acc-map-fallback-v16">${map.fallback}</span>
        <img class="acc-map-logo-v16" alt="" decoding="async" loading="eager">
      </div>
    ...
    const img = button.querySelector(".acc-map-logo-v16");
  ...
  function render(){
    ...
    if(root.dataset.mapsRevision !== REVISION){
      ...
      root.dataset.logoMode = "INDIVIDUAL_FILES_V16";
  ...
  window.ACCMyMaps = Object.freeze({
    revision: REVISION,
    spriteSource: "NONE_V16_INDIVIDUAL_FILES",
