...
(() => {
  ...
  const patchText = () => {
    document.querySelectorAll(".build").forEach(node => {
      ...
      if (/Build\s+(?:250|256)\s*•/i.test(value)) node.textContent = `Build ${RELEASE.build} • Core ${RELEASE.corePackageLabel} • KAI System ${RELEASE.kaiSystemLabel}`;
    });
    document.querySelectorAll(".badge").forEach(node => {
      const value = String(node.textContent || "").trim().toUpperCase();
      if (value === "BUILD 250" || value === "BUILD 256") node.textContent = `BUILD ${RELEASE.build}`;
    });
    document.querySelectorAll(".item.row.between").forEach(row => {
      const label = row.querySelector("span"), value = row.querySelector("strong");
      if (label && value && String(label.textContent || "").trim() === "New Version") value.textContent = String(RELEASE.build);
    });
    const boot = document.querySelector("#boot .boot-sub");
    if (boot && /BUILD\s+(?:250|256)/i.test(String(boot.textContent || ""))) boot.textContent = `ACC OS X • BUILD ${RELEASE.build}`;
  };
  ...
  const queuePatch = () => {
    if (queued) return;
    queued = true;
    requestAnimationFrame(() => { queued = false; patchText(); });
  };
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (method === "GET" && /(?:^|\/)version\.json(?:\?|$)/.test(url)) {
      ...
      try {
        ...
        return new Response(JSON.stringify({...data,version:RELEASE.dataSchema,appBuild:RELEASE.build,appRevision:RELEASE.revision,dataSchema:RELEASE.dataSchema,corePackageRevision:RELEASE.corePackageRevision,kaiSystemRevision:RELEASE.kaiSystemRevision}), {status: response.status, statusText: response.statusText, headers});
